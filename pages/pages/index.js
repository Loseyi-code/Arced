...
function TypeWriter() {
  ...
  return (
    <div style={{textAlign:'center',marginBottom:12}}>
      <span style={{fontFamily:'Syne,sans-serif',fontSize:'clamp(60px,10vw,96px)',fontWeight:900,letterSpacing:-4,background:'linear-gradient(130deg,#fff 0%,#4db8ff 45%,#1a8cff 80%,#f0c060 100%)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text',display:'inline'}}>{text}</span>
      <span style={{display:'inline-block',width:5,height:'0.8em',background:done?'transparent':'#4db8ff',marginLeft:4,verticalAlign:'middle',borderRadius:1,animation:'cblink 0.8s step-end infinite'}}/>
    </div>
  );
}
      ...
      <svg style={{transform:'rotate(-90deg)',width:240,height:240}} viewBox="0 0 240 240">
        <defs>
          <linearGradient id="hg" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#1a8cff"/><stop offset="100%" stopColor="#4db8ff"/>
          </linearGradient>
          <linearGradient id="sg" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#4a6080"/><stop offset="100%" stopColor="#7088aa"/>
          </linearGradient>
        </defs>
        <circle cx="120" cy="120" r={r} fill="none" stroke="rgba(26,140,255,0.07)" strokeWidth="10"/>
        <circle cx="120" cy="120" r={r} fill="none" stroke={happy?'url(#hg)':'url(#sg)'} strokeWidth="10" strokeLinecap="round"
          strokeDasharray={circ} strokeDashoffset={offset}
          style={{transition:'stroke-dashoffset 2.5s cubic-bezier(.2,.8,.2,1)',filter:happy?'drop-shadow(0 0 9px rgba(77,184,255,.9))':'drop-shadow(0 0 7px rgba(112,136,170,.7))'}}/>
      </svg>
  ...
  return (
    <>
      ...
      <style>{`
        ...
        @media(max-width:540px){.sgrid{grid-template-columns:1fr!important}.irow{flex-direction:column!important}.sbtn{width:100%!important}}
      `}</style>
      ...
      <main style={{position:'relative',zIndex:1,maxWidth:860,margin:'0 auto',padding:'44px 20px 100px'}}>
        <div style={{textAlign:'center',marginBottom:52,animation:'fdown .9s ease both'}}>
          <div style={{fontFamily:'Space Mono,monospace',fontSize:10,letterSpacing:5,color:'#1a8cff',textTransform:'uppercase',marginBottom:20,opacity:.65}}>// arc network — architect scanner v2.0</div>
          ...
          <TypeWriter/>
          <div style={{fontSize:15,fontWeight:300,color:'rgba(168,216,255,.42)',letterSpacing:.5}}>Enter your X handle — see exactly how engaged you are with Arc.</div>
        </div>
        <div style={{background:'rgba(4,30,56,0.72)',border:'1px solid rgba(26,140,255,0.18)',borderRadius:8,padding:28,marginBottom:16,position:'relative',overflow:'hidden',backdropFilter:'blur(12px)',animation:'fup .9s ease .2s both'}}>
          ...
          <label style={{fontFamily:'Space Mono,monospace',fontSize:10,letterSpacing:4,color:'#4db8ff',textTransform:'uppercase',marginBottom:14,display:'block',opacity:.65}}>// enter your x username</label>
          <div className="irow" style={{display:'flex',gap:10,alignItems:'stretch'}}>
            ...
            <input value={username} onChange={e=>setUsername(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleScan()} placeholder="yourhandle" style={{flex:1,background:'rgba(26,140,255,.06)',border:'1px solid rgba(26,140,255,.22)',borderRadius:4,padding:'15px 20px',color:'#d6eeff',fontFamily:'Space Mono,monospace',fontSize:15,letterSpacing:1}}/>
            <button className="sbtn" onClick={handleScan} disabled={loading} style={{background:'linear-gradient(135deg,#1a8cff,#4db8ff)',border:'none',borderRadius:4,padding:'15px 32px',color:'#010c18',fontFamily:'Syne,sans-serif',fontSize:13,fontWeight:800,letterSpacing:2,cursor:'pointer',transition:'all .3s',textTransform:'uppercase'}}>{loading?'...':'SCAN'}</button>
          </div>
          {error&&<div style={{marginTop:10,background:'rgba(255,60,60,.07)',border:'1px solid rgba(255,60,60,.25)',borderRadius:4,padding:'10px 16px',fontFamily:'Space Mono,monospace',fontSize:11,color:'#ff8080'}}>⚠ {error}</div>}
        </div>
        {loading&&(
          <div style={{padding:'52px 0',textAlign:'center'}}>
            <div style={{fontFamily:'Space Mono,monospace',fontSize:11,letterSpacing:3,color:'#4db8ff',textTransform:'uppercase',animation:'pulse 1.1s ease infinite',marginBottom:22}}>{STEPS[step]}</div>
        ...
        {result&&!loading&&(
          <div style={{animation:'fup .7s ease both'}}>
            <div style={{background:'rgba(4,30,56,0.72)',border:'1px solid rgba(26,140,255,0.18)',borderRadius:8,padding:'20px 24px',marginBottom:14,display:'flex',alignItems:'center',gap:16,backdropFilter:'blur(12px)',flexWrap:'wrap'}}>
              {result.user.avatar&&<img src={result.user.avatar} alt={result.user.name} style={{width:52,height:52,borderRadius:'50%',border:'2px solid rgba(26,140,255,.3)'}}/>}
              <div style={{flex:1}}>
            ...
            <div style={{background:'rgba(4,30,56,0.72)',border:'1px solid rgba(26,140,255,0.18)',borderRadius:8,padding:'44px 28px 38px',marginBottom:14,textAlign:'center',position:'relative',overflow:'hidden',backdropFilter:'blur(12px)'}}>
              ...
              <div style={{fontFamily:'Space Mono,monospace',fontSize:12,letterSpacing:4,color:'rgba(77,184,255,.5)',marginBottom:28}}>@{result.user.username}</div>
              <ScoreRing score={result.analysis.score} happy={happy}/>
              <div style={{display:'inline-flex',alignItems:'center',gap:8,fontFamily:'Syne,sans-serif',fontSize:12,fontWeight:700,letterSpacing:3,padding:'7px 24px',borderRadius:3,textTransform:'uppercase',marginBottom:14,background:`${rank.color}22`,color:rank.color,border:`1px solid ${rank.color}55`}}>{rank.emoji} {rank.label}</div>
              <br/>
              <div style={{fontSize:15,color:'rgba(214,238,255,.42)',fontWeight:300,maxWidth:420,margin:'0 auto',lineHeight:1.65}}>{happy?'🎉 Congrats! You are genuinely engaged with Arc. Keep building!':'😢 Not much Arc engagement found. Time to dive into the ecosystem!'}</div>
            </div>
            <div style={{fontFamily:'Space Mono,monospace',fontSize:10,letterSpacing:4,color:'#4db8ff',textTransform:'uppercase',marginBottom:12,opacity:.45}}>// engagement breakdown</div>
            ...
            {result.analysis.matchedTweets?.length>0&&(
              <>
                <div style={{fontFamily:'Space Mono,monospace',fontSize:10,letterSpacing:4,color:'#4db8ff',textTransform:'uppercase',marginBottom:12,opacity:.45}}>// arc-related tweets</div>
            ...
            <div style={{background:'rgba(4,30,56,0.72)',border:'1px solid rgba(26,140,255,0.18)',borderRadius:8,padding:26,textAlign:'center',backdropFilter:'blur(12px)'}}>
              <div style={{fontFamily:'Syne,sans-serif',fontSize:13,fontWeight:700,letterSpacing:3,color:'#4db8ff',marginBottom:18}}>// BROADCAST YOUR ARC STATUS</div>
              <button className="share-btn" onClick={()=>{const emoji=happy?'🎉':'😢';const txt=`I just checked my Arc status ${emoji}\n\nI'm ${result.analysis.score}% ARCED on @arc_network\n\n${result.analysis.arcMatches} Arc engagements found!\n\nCheck yours 👇`;window.open('https://twitter.com/intent/tweet?text='+encodeURIComponent(txt),'_blank');}} style={{background:'transparent',border:'1.5px solid #4db8ff',borderRadius:4,padding:'13px 32px',color:'#4db8ff',fontFamily:'Syne,sans-serif',fontSize:13,fontWeight:700,letterSpacing:2,cursor:'pointer',transition:'all .3s',textTransform:'uppercase'}}>SHARE ON X →</button>
            </div>
          </div>
        )}
        <div style={{textAlign:'center',marginTop:56,fontFamily:'Space Mono,monospace',fontSize:10,color:'rgba(26,140,255,.18)',letterSpacing:3}}>ARC NETWORK — ARCHITECTS PROGRAM — COMMUNITY TOOL</div>
      </main>
    </>
  );
}
